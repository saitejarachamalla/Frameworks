...
public class Add_Product_To_cart extends BaseTestDemo {
	...
	public static void ToMyAccount() {
		try {
	...
	public static void GotoAddCart() {
		try {
			Assert.assertTrue(addcart0.isDisplayed()&&addcart0.isEnabled());
			getExtentTest().log(LogStatus.PASS, "addcart0 is displayed");
			
			String cartvalue = addcart0.getAttribute("value");
			int crtval = Integer.parseInt(cartvalue);
			if (crtval == 0) {
				getExtentTest().log(LogStatus.PASS, "cart value = 0");
				
			} else {
				getExtentTest().log(LogStatus.FAIL, "cart value NOT = 0");
			}
